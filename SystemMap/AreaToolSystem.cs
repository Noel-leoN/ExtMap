	...
	public partial class AreaToolSystem : ToolBaseSystem
	{
		...
		protected override JobHandle OnUpdate(JobHandle inputDeps)
		{
			...
			if (this.prefab != null)
			{
				...
				this.m_AllowCreateArea = base.m_ToolSystem.actionMode.IsEditor() || componentData.m_Type != Game.Areas.AreaType.Lot;
				base.UpdateInfoview(base.m_ToolSystem.actionMode.IsEditor() ? Entity.Null : base.m_PrefabSystem.GetEntity(this.prefab));
				Systems.AreaToolSystem.GetAvailableSnapMask(componentData, base.m_ToolSystem.actionMode.IsEditor(), out base.m_SnapOnMask, out base.m_SnapOffMask);
		...
		private void UpdateActions()
		{
			...
			this.m_ApplyAction.SetDisplayProperties("Add/Modify Area Node", 20);
			this.m_SecondaryApplyAction.shouldBeEnabled = true;
			if (this.m_State == State.Create && this.m_ControlPoints.Length > 1)
			{
				this.m_SecondaryApplyAction.SetDisplayProperties("Undo Area Node", 25);
			}
			else
			{
				this.m_SecondaryApplyAction.SetDisplayProperties("Remove Area Node", 25);
			}
		}
