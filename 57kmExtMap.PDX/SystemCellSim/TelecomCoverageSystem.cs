		...
		public struct TelecomCoverageJob : IJob
		{
			...
			private void AddNetworkCapacity(NativeArray<CellDensityData> densityData, NativeArray<CellFacilityData> facilityData, NativeList<float> signalStrengths, ArchetypeChunk chunk, ref int arrayIndex, ref TelecomStatus status, DynamicBuffer<CityModifier> cityModifiers)
			{
				...
				for (int i = 0; i < nativeArray.Length; i++)
				{
					...
					{
						int2 @int = math.max(CellMapSystem<TelecomCoverage>.GetCell(transform.m_Position - result.m_Range, CellMapSystem<TelecomCoverage>.kMapSize, 128), 0);
						int2 int2 = math.min(CellMapSystem<TelecomCoverage>.GetCell(transform.m_Position + result.m_Range, CellMapSystem<TelecomCoverage>.kMapSize, 128) + 1, 128);
						int2 int3 = int2 - @int;
						if (!math.any(int3 <= 0))
						{
			...
			private void CalculateSignalStrength(NativeArray<CellFacilityData> facilityData, NativeArray<float> obstructSlopes, NativeList<float> signalStrengths, ArchetypeChunk chunk, DynamicBuffer<CityModifier> cityModifiers)
			{
				...
				for (int i = 0; i < nativeArray.Length; i++)
				{
					...
					int2 @int = math.max(CellMapSystem<TelecomCoverage>.GetCell(position - result.m_Range, CellMapSystem<TelecomCoverage>.kMapSize, 128), 0);
					int2 int2 = math.min(CellMapSystem<TelecomCoverage>.GetCell(position + result.m_Range, CellMapSystem<TelecomCoverage>.kMapSize, 128) + 1, 128);
					int2 int3 = int2 - @int;
					if (math.any(int3 <= 0))
					{
						continue;
